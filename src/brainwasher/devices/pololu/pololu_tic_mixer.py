...
class TicCmd:
    ...
    def get_step_mode(self) -> int:
        step_mode = int(self._parse_status("Step mode:"))
        logging.debug(f"Step mode is {step_mode}.")
        return step_mode

    def get_target_velocity(self) -> int:
        velocity = int(self._parse_status("Target velocity:"))
        logging.debug(f"Target velocity is {velocity}.")
        return velocity
...
class PololuTicMixer(Mixer):
    ...
    def _start_mixing(self):
        if (v := self.tic.get_target_velocity()) != 0:  # don't start if speed is set to 0
            logging.debug(f"Starting mixing as speed {v} steps.")
            self.tic.energize()
