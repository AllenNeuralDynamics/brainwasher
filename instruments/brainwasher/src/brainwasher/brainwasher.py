...
def lock_flowpath(func):
    ...
    def inner(self, *args, **kwds):
        with self.flowpath_lock:
            self.log.debug(
                f"Locking flowpath to {current_thread().name} for {func.__name__} fn."
            )
...
def syringe_empty(func):
    ...
    def inner(self, *args, **kwds):
        ...
        if abs(pump_position_ul) > self.PUMP_APPROX_ZERO_UL:
            error_msg = (
                "Error. Pump is not starting from its reset position (~0) "
...
class BrainWasher:
    ...
    def __init__(
        ...
        self.waste_drain_valves = (
            waste_drain_valves  # reaction vessel waste drain valve.
        )
    ...
    def _validate_setup(self):
        ...
        if not (selector_lds_map_keys <= selector_port_map_keys):
            raise RuntimeError(
                "Key names in selector port map and bds map do not match!"
            )
        ...
        if not (required_ports <= selector_port_map_keys):
            raise RuntimeError(
                "The selector port map must include the "
    ...
    def start_pressure_monitor(self):
        ...
        self.pressure_monitor_thread = Thread(
            target=self._monitor_pressure_worker,
    ...
    def _monitor_pressure_worker(self):
        ...
        while self.monitoring_pressure.is_set():
            ...
            if self.buffer_samples.is_set():
                ...
                if (
                    now() - self.pressure_avg_start_time_s
    ...
    def get_compatible_waste_vessel_id(
        self, *chemicals: str, waste_vessels: list[WasteVessel] = None
    ) -> int | None:
        ...
        if not chemicals:
            self.log.warning(
                "Reaction vessel is empty. Any waste vessel is compatible."
            )
        # A solution is chemically compatible if its components are a subset
        # of the vessel's compatible components.
        waste_compatibility = [
            set(chemicals) <= wv.compatible_chemicals for wv in waste_vessels
        ]
    ...
    @lock_flowpath
    @syringe_empty
    def prime_reservoir_line(
        self, chemical: str, max_pump_displacement_ul: int = 12500
    ):
        ...
        if (not SIMULATED) and self.selector_lds_map[chemical].tripped():
            self.log.warning(
                f"{chemical} reservoir line detected prematurely as primed. Aborting."
            )
        ...
        self.log.debug("Opening pump path to waste.")
        self.rv_source_valve.deenergize()
        self.rv_exhaust_valve.deenergize()
        ...
        while (not liquid_detected) and (remaining_volume_ul > 5):
            ...
            self.log.debug(
                "Polling prime-reservoir sensor while withdrawing up to "
        ...
        self.log.info(
            f"Priming {chemical} complete. Function displaced "
            f"{displaced_volume_ul:.3f}[uL] of volume."
        )

    @lock_flowpath
    @syringe_empty
    def unprime_reservoir_line(
        self, chemical: str, max_pump_displacement_ul: int = 25000
    ):
        ...
        if chemical not in self.prime_volumes_ul:
            self.log.warning(
                f"{chemical} has never been primed before. "
                f"Unpriming will displace {max_pump_displacement_ul}[uL]."
            )
        unprime_volume_ul = self.prime_volumes_ul.get(
            chemical, max_pump_displacement_ul
        )
        # Add 5% for good measure, but stay below alotted maximum.
        unprime_volume_ul = min(
            unprime_volume_ul * 1.05,  # FIXME: magic number
    ...
    def prime_pump_line(self, chemical: str):
        ...
        chemical."""
        if SIMULATED:
            self.log.warning("Skipping priming pump in simulation.")
        ...
        if self.pump_is_primed_with:
            ...
            self.log.warning(
                f"Pump line already primed with {self.pump_is_primed_with}."
            )
            return
        ...
        self.pump.log.setLevel(logging.INFO)  # Unset Debug level (if set) for pump.
        self.pump.withdraw(
            self.pump.syringe_volume_ul / 3, wait=False
        )  # FIXME: magic number
        while self.pump.is_busy():
            ...
            self.pump.halt()
            self.log.debug(
                "Priming pump line detected liquid after displacing "
            ...
            return
        raise RuntimeError(
            f"Did not detect any liquid ({chemical}) after "
    ...
    def purge_pump_line(
        ...
        try:
            ...
            if full_cycles:
                self.log.debug(
                    "Pulling residual pump line contents into syringe with N2."
                )
                ...
                while remaining_volume_ul > self.PUMP_APPROX_ZERO_UL:
                    ...
                    while self.pump.is_busy():
                        ...
                        if (now() - start_time_s) > self.PRESSURE_POCKET_TIMEOUT_S:
                            self.log.error(
                                f"Pump timed out (i.e: thinks it is "
    ...
    def dispense_to_vessel(self, microliters: float, chemical: str):
        ...
        self.pump.move_absolute_in_percent(0)
        self.log.debug(
            f"Plunging remaining {pump_to_common_dv_ul}[uL]"
        ...
        self.purge_pump_line(
            self.pump_is_primed_with, destination=self.rxn_vessel, gas_cycles=1
        )
        ## Update State:
        ...
        self.output_bypass_valves[waste_id].deenergize()
        self.log.debug(
            f"Dispensed {microliters}[uL] into reaction vessel. "
    ...
    def drain_vessel(self, drain_volume_ul: float = 40000):
        ...
        msg = (
            "Draining vesssel" + f" of {self.rxn_vessel.solution}."
            if self.rxn_vessel.solution
            else "."
        )
        self.log.info(msg)
        ...
        self.log.debug(
            f"Waste contents will be discarded to {self.waste_vessels[waste_id].name}."
        )
        # Set outlet flowpath starting configuration.
        self.rv_source_valve.energize()
        ...
        while remaining_volume_ul:
            # Withdraw another stroke.
            stroke_volume_ul = min(remaining_volume_ul, syringe_volume_ul)
            stroke_percent = stroke_volume_ul / syringe_volume_ul * 100.0
    ...
    def run_wash_step(
        ...
        **solution: dict,
        ...
        slow_mix_times = [intermittent_mixing_on_time_s, intermittent_mixing_off_time_s]
        ...
        if mix_speed_rpm > 0:
            try:
                ...
                self.log.warning(
                    "Mixer does not support speed control. Skipping speed setting."
                )
        ...
        while (now() - start_time_s) < duration_s:
            ...
            if (
                self.job_worker
                and self.job_worker.is_alive()
                and self.pause_requested.is_set()
            ):
                ...
                self.resume_state_overrides.update(
                    duration_s=(duration_s - elapsed_time_s)
                )
                return
    ...
    def mix(
        self,
        duration_s: int,
        mix_speed_rpm: float = 1000,
        intermittent_mixing_on_time_s: float = None,
        intermittent_mixing_off_time_s: float = None,
    ):
        self.run_wash_step(
            duration_s=duration_s,
            mix_speed_rpm=mix_speed_rpm,
    ...
    def create_job(self, job_path: str, source: str = None):
        ...
        if not job_path.exists():
            ...
            self.log.info("Created an empty job file.")
            return
        ...
        if source_path.suffix.lower() in ["yaml", "yml"]:
            with open(source_path) as source_job, open(job_path) as job_file:
                ...
                yaml.dump(job.model_dump(exclude_none=True), job_file)
                self.log.info("Created job file from an existing job file.")
                return
    ...
    def validate_job_against_instrument(self, job: Job):
        ...
        for index, step in enumerate(job.protocol):
            if step.solution_volume_ul > self.rxn_vessel.max_volume_ul:
                msg = (
                    f"Step {index}: solution total volume "
        ...
        if volume_errors:
            raise ValueError(
                "Job volumes are not compatible with the size "
        ...
        if not job.chemicals <= self.plumbed_chemicals:
            raise ValueError(
                f"Job chemicals are not plumbed on the machine; "
        ...
        for index, step in enumerate(job.protocol):
            ...
            if solution_has_valid_waste is None:
                msg = (
                    f"Step {index} solution has no designated waste. "
        ...
        if waste_compatibility_errors:
            raise ValueError(
                "Job is not chemically compatible with waste "
        ...
        for index, step in enumerate(job.protocol):
            waste_vessel_id = self.get_compatible_waste_vessel_id(
                *step.components, waste_vessels=waste_vessels
            )
            # Raises an error if we're at capacity.
            waste_vessels[waste_vessel_id].add_solution(**step.solution)
        self.log.info("Job passed validation against instrument capabilities.")
    ...
    def _run_job_worker(self, job: Job, job_path: Path):
        ...
        if job.resume_state:
            ...
            if self.rxn_vessel.solution != job.resume_state.starting_solution:
                raise ValueError(
                    "When resuming, reaction vessel starting "
            ...
            if self.rxn_vessel.solution != job.starting_solution:
                raise ValueError(
                    "When starting, reaction vessel starting "
                    "solution does not match the correct resume "
                    "state starting solution."
                )
            job.record_start()
        ...
        if start_step > 0:
            ...
            log_msg += ". "
        log_msg += (
            f"Job should take {timedelta(seconds=job.get_duration_s(start_step))}."
        )
        ...
        for index, step in enumerate(job.protocol[start_step:], start=start_step):
            ...
            try:
                ...
                if index == start_step and start_step_overrides:
                    ...
                    self.log.info(
                        f"Applying overrides to starting step: {start_step_overrides}."
                    )
                ...
                job.save_resume_state(
                    resume_step, step.solution, **self.resume_state_overrides
                )
                self.resume_state_overrides = {}
    ...
    def leak_check_syringe_to_rv_exaust_normally_open_path(self):
        try:
            ...
            self._purge_gas_filled_syringe()
        self.log.info(
            "leak check passed: syringe -><- reaction vessel exhaust NO path."
        )
    ...
    def leak_check_syringe_to_waste_bypass_path(self):
        try:
            ...
            msg = (
                "Flowpath between syringe pump and closed output bypass valve"
                "is leaking."
            )
            self.log.error(msg)
            raise
    ...
    def leak_check_syringe_to_reaction_vessel(self):
        try:
            ...
            msg = "Flowpath between syringe pump and sealed reaction vesselis leaking."
            self.log.error(msg)
            raise
    ...
    @lock_flowpath
    def _squeeze_and_measure(
        self, pump_compression_percent: float = None, measurement_time_s: float = 4.0
    ):
        ...
        pump_compressed_position_percent = (
            pump_position_percent - pump_compression_percent
        )
        ...
        if (
            compressed_pressure - uncompressed_pressure
        ) < self.MIN_LEAK_CHECK_STARTING_PRESSURE_PSIG:
            raise LeakCheckError(
                "Syringe cannot create a positive relative "
        ...
        while now() - start_time_s < measurement_time_s:
            ...
            if delta > self.MAX_LEAK_CHECK_PRESSURE_DELTA_PSIG:
                raise LeakCheckError(
                    "Pressure change is significant enoughto indicate a leak."
                )
