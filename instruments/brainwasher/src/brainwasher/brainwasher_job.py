...
class BrainwasherResumeState(BaseModel):
    ...
    def values_in_wash_step(overrides: dict):
        ...
        if not override_keys.issubset(valid_wash_step_model_fields):
            raise ValueError(
                f"Override fields must all be valid WashStep "
        ...
        try:  # Lazy way: try making a valid Step.
            ...
            extra_msg = (
                "Error validating ResumeState overrides against WashStep values."
            )
    ...
    # those in a WashStep.
    overrides: Annotated[
        Optional[dict[str, Any]], AfterValidator(values_in_wash_step)
    ] = None
