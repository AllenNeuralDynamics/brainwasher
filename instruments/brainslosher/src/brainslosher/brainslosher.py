...
class BrainSlosher(Instrument):
    ...
    def get_progress(self) -> int:
        ...
        if (
            not overrides
            or not overrides.get("duration_min")
            or not overrides.get("washes")
        ):
            return
        ...
        elapsed_minutes = sum(
            step.washes * step.duration_min for step in protocol[:curr_step_index]
        )

        # current step partial progress
        current_step = protocol[curr_step_index]
        elapsed_minutes += (
            (current_step.washes - remaining_washes) * current_step.duration_min
        ) + (current_step.duration_min - remaining_duration)
    ...
    def fill_chamber(self, solution: str, volume_ml: float) -> None:
        ...
        if (
            self.rxn_vessel.curr_volume_ul + volume_ml * 1000
            > self.rxn_vessel.max_volume_ul
        ):
    ...
    def drain_chamber(self, volume_ml: float = None) -> None:
        ...
        if volume_ml + self.waste.curr_volume_ul > self.waste.max_volume_ul:
            raise ValueError(
                "Waste vessel will exceed max volume if chamber is drained. Please empty."
            )

        self.log.info(f"Draining chamber of {volume_ml}mL")
        self.withdraw_and_dispense_solution(
            "drain", volume_ml + self.config.drain_volume_buffer_ml, "waste"
        )
        self.waste.add_solution(**self.rxn_vessel.solution)
        self.rxn_vessel.purge_solution()

    @lock_flowpath
    def withdraw_and_dispense_solution(
        self, solution: str, volume_ml: float, dispense_to: Literal["waste", "chamber"]
    ) -> None:
        """
        ...
        while volume_ml > 0:
            ...
            self.log.debug(
                f"Moving pump to {solution} valve {self.config.selector_port_map[solution]}."
            )
            ...
            volume_ml -= pump_vol
        self.log.debug(
            f"finished dispensing {volume_ml}mL of {solution} to {dispense_to}"
        )
    ...
    def prime_line(self, solution: str) -> None:
        ...
        self.withdraw_and_dispense_solution(
            solution, self.config.prime_volume_ml, "waste"
        )
    ...
    def purge_line(self) -> None:
        ...
        self.withdraw_and_dispense_solution(
            "air", self.config.purge_volume_ml, "chamber"
        )
        self.pump.reset_syringe_position()
    ...
    def _run_job_worker(self, job: BrainSlosherJob, job_path: Path):
        ...
        try:
            self.log.info(
                f"Job starting. Job estimated finish at {timedelta(seconds=job.get_duration_s()) + datetime.now()}"
            )
            with self.job_status_lock:
    ...
    def save_resume_state(
        self, job: BrainSlosherJob, resume_step: int, starting_solution: str, **kwargs
    ):
        """
        ...
        job.save_resume_state(
            resume_step,
            {starting_solution: self.config.fill_volume_ml * 1000},
            **kwargs,
        )
    ...
    def run_wash_step(self, duration_min: float, solution: str):
        ...
        if solution not in self.config.selector_port_map.keys():
            raise ValueError(
                f"Solution {solution} is not currently plumbed based on config."
            )

        self.drain_chamber()
        self.prime_line(solution)
        self.fill_chamber(solution, self.config.fill_volume_ml)
        self.purge_line()
        ...
        while (perf_counter() - start_time_s) < duration_s:
            ...
            self.resume_state_overrides.update(
                duration_min=duration_min - round(elapsed_min, 3)
            )  # updated outside of pause so overrides always has upto date overrides
    ...
    def save_job(self, job: dict):
        ...
        while job_path.exists():
            job_path = (
                Path(self.config.save_folder) / f"{valid_job.name}_{counter}.yaml"
            )
            counter += 1
