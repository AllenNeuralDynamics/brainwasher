...
class BrainSlosherResumeState(BaseModel):
    ...
    def values_in_cycle(overrides: dict):
        ...
        if not override_keys.issubset(valid_cycle_model_fields):
            raise ValueError(
                f"Override fields must all be valid Cycle "
    ...
    overrides: Annotated[Optional[dict[str, Any]], AfterValidator(values_in_cycle)] = (
        None
    )
...
class BrainSlosherJob(Job):
    ...
    def get_duration_s(self, start_step: int = 0):
        ...
        return sum(
            [
                step.duration_min * step.washes * 60
    ...
    def save_resume_state(
        ...
        self.resume_state = BrainSlosherResumeState(
            step=step, starting_solution=starting_solution, overrides=overrides
        )
...
class BrainSlosherConfig(
    BaseModel,
    validate_assignment=True,
):
    save_folder: Path = Field(default="../../brain_slosher_jobs/")
    selector_port_map: dict[str, int]
    max_syringe_volume_ml: float = Field(
        default=4.5,
        description="Maximum fill volume of the syringe to prevent chatter when operating.",
    )
    prime_volume_ml: float = Field(default=11, description="Volume to prime lines.")
    purge_volume_ml: float = Field(
        default=4.5, description="Volume to purge drain line."
    )
    drain_volume_buffer_ml: float = Field(
        ...,
        description="Buffer to add to draining volume to ensure chamber is completly empty.",
    )
    fill_volume_ml: float = Field(..., description="Volume to fill chamber completly.")
    user_email: Optional[EmailStr] = Field(
        default=None, description="Optional email to send errors to."
    )  # validates email with email-validator package
    ...
    def check_required_keys(cls, v: dict[str, int]):
        ...
        if "air" not in v:
            raise ValueError(
                "selector_port_map must contain an 'air' key for purging line."
            )
