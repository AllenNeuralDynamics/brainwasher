...
class ZMQServer(RouterServer):
    def __init__(
        ...
        self.add_named_call(
            ...
            "brainslosher",
            "drain_chamber",
            kwargs={"volume_ml": self.brainslosher.rxn_vessel.max_volume_ul / 1000},
        )  # fully drain vessel since ui does not know current fill
        ...
        self.add_named_call(
            "set_drain_buffer_volume", "brainslosher", "set_drain_buffer_volume"
        )
    ...
    def check_job_status(self) -> dict:
        ...
        if message.status == "finished":
            if self.brainslosher.config.user_email:
                send_email(
                    subject="Brainslosher job is done!",
            ...
            if self.brainslosher.config.user_email:
                send_email(
                    subject="Error during brainslosher job!",
...
def main():
    ...
    def record_factory(*args, **kwargs):
        ...
        prefix = (
            f"{brainslosher.config.instrument_name}: "
            if brainslosher.config.instrument_name
            else ""
        )
    ...
    # start server
    server = ZMQServer(
        instances={"brainslosher": brainslosher},
